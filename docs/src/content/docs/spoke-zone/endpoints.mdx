...
| Success body | `200`: `{ status, message }`; `201`: `{ token }` |
...
| Success status | `200` |
| Success body | `token` (string), `expires` (number), `user` object (`username`, `user_id`, `user_role`, `customer_id`, `customer_role`, `oem_id`, `reset_required`, `customer_prefix`, `is_mobile_user`, `prefix`, `mqtt_permission`) |
...
| Success status | `200` |
| Success body | list of items with `id` (number), `modelId` (number), `moduleId` (number), `module` (string), `version` (string), `fileLocation` (string), `isActive` (boolean), `createdDate` (string), `releaseNotes` (string) |
| Error statuses to map | `400`, `401`, `403`, `429`, `5xx` |
